...
if (isset($_GET['updated'])) {
    $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                <strong>Succès !</strong> Les données de la facture ont été mises à jour.
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        ...
        $message = '<div class="alert alert-danger">Erreur lors de la suppression de la facture : ' . htmlspecialchars($e->getMessage()) . '</div>';
    }
}

// 3. FILTRES, RECHERCHE ET TRI
...
<head>
    ...
    <title>Liste des factures — NumériqueAide</title>
...
<div class="container-fluid px-4 mt-4 mb-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="add_facture.php" class="btn btn-success">
            <i class="bi bi-file-earmark-plus me-1"></i> Créer une facture
        </a>
    </div>
            ...
            <form method="GET" class="row g-2">
                ...
                <div class="col-md-7">
                    <input type="text" name="search" class="form-control"
                           placeholder="Recherche par numéro de facture ou nom du client..."
                ...
                <div class="col-md-2 d-flex gap-2">
                    <button type="submit" class="btn btn-primary w-100">
                        <i class="bi bi-filter"></i> Trouver
                    </button>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover table-sm table-custom-sm align-middle mb-0">
                    ...
                    <?php if (count($factures) > 0): ?>
                        <?php foreach ($factures as $f): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($f['client_nom'] ?? 'Non spécifié'); ?></td>
                                ...
                                <td class="text-end">
                                    <div class="btn-group btn-group-sm" role="group">
                                        <a href="edit_facture.php?id=<?= $f['id']; ?>" class="btn btn-outline-primary" title="Modifier">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <?php if (!empty($f['pdf_path'])): ?>
                                            <a href="<?= htmlspecialchars($f['pdf_path']); ?>" class="btn btn-outline-dark" title="Télécharger le PDF" target="_blank">
                                                <i class="bi bi-file-pdf"></i>
                                            </a>
                                        <?php else: ?>
                                            <a href="facture_pdf.php?id=<?= $f['id']; ?>" class="btn btn-outline-dark" title="Générer le PDF" target="_blank">
                                                <i class="bi bi-file-pdf"></i>
                                            </a>
                                        <?php endif; ?>
                                        <a href="<?= basename($_SERVER['PHP_SELF']); ?>?delete_id=<?= $f['id']; ?>"
                                           ...
                                           title="Supprimer"
                                           onclick="return confirm('Êtes-vous sûr de vouloir supprimer la facture n° <?= htmlspecialchars($f['facture_number']); ?> ?');">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="8" class="text-center py-4 text-muted">Aucune facture trouvée.</td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
                </table>
